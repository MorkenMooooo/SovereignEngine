...
static void display_sovereign_image(spi_device_handle_t spi)
{
    uint16_t *lines[2];
    void *raw_buf[2]; // 保存原始指针用于释放

    for (int i = 0; i < 2; i++) {
        size_t size = 240 * PARALLEL_LINES * sizeof(uint16_t);
        // 分配额外 3 字节用于对齐
        raw_buf[i] = heap_caps_malloc(size + 3, MALLOC_CAP_DMA);
        assert(raw_buf[i] != NULL);

        // 手动 4 字节对齐
        uintptr_t addr = (uintptr_t)raw_buf[i];
        lines[i] = (uint16_t*)((addr + 3) & (~3));
    }
    ...
    while (1) {
        for (int y = 0; y < 320; y += PARALLEL_LINES) {
            for (int i = 0; i < 240 * PARALLEL_LINES; i++) {
                uint16_t pixel = g_sovereign_image[y * 240 + i];
                lines[0][i] = (pixel << 8) | (pixel >> 8); // 字节交换
            }
            if (sending_line != -1) {
                send_line_finish(spi);
            }
